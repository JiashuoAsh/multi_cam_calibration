...
# 用法: bash scripts/run_calibration_pipeline.sh
...
echo ""
echo "[Pipeline] 运行 python -m mcca.entry.pipeline ..."
python -m mcca.entry.pipeline --all --config config/apriltag_config.json | tee output.txt
if [ $? -ne 0 ]; then
    echo "[FAIL] 流水线失败" >&2
    exit 1
fi
...
echo "  - 外参结果: results/multi_camera_extrinsics.json（2+ 相机，包含双目）"
