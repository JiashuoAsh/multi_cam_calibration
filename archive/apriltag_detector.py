#!/usr/bin/env python3
"""
旧的多尺度AprilTag检测库 --- 没有亚像素角点优化（重要），缺乏opencv封装库的调用（鸡肋）
...
使用方法：
  ...
  # 方式2：使用完整的类（支持可视化）
  from apriltag_detector import AdvancedAprilTagDetector
...
class AdvancedAprilTagDetector:
    ...
    def _detect_on_image(self, img: np.ndarray):
        ...
        2. 轮廓检测 → 找到所有封闭形状
        3. 多边形拟合 → 筛选出四边形
        4. 周长/面积过滤 → 使用DetectorParameters中的阈值
        5. 透视变换 → 将候选四边形"拉正"
        6. 位模式解码 → 读取内部黑白格子，解码为ID
        7. 字典匹配 → 验证ID是否在字典中
        8. 亚像素优化 → 精细化角点位置（cornerSubPix）

        Args:
            img: 输入图像 (灰度，uint8)

        Returns:
            corners: List[np.ndarray], 每个元素形状(1,4,2), 表示一个标签的4个角点
                     角点顺序: 左上→右上→右下→左下（逆时针）
            ids: np.ndarray, 形状(N,1), 检测到的标签ID
            rejected: List[np.ndarray], 被拒绝的候选四边形（形状正确但解码失败）

        示例:
            corners[0] = [[[100, 200],   # 左上角
                          [150, 200],   # 右上角
                          [150, 250],   # 右下角
                          [100, 250]]]  # 左下角
            ids[0] = [5]  # 标签ID为5
    ...
    def detect_multiscale_simple(
        ...
        返回格式与 cv2.aruco.detectMarkers 兼容
        """
        if len(image.shape) == 3:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        else:
        ...
        if len(all_detections) > 0:
            ...
            # ==== 角点顺序修正（与utils.py保持一致）====
            # 多尺度检测也需要应用相同的180°旋转修正
            # 详细说明见 utils.py 中的 detect_apriltag_corners 函数
...
def detect_apriltag_multiscale(
    ...
    用法：
        # 替换原有的检测代码
        corners, ids = detect_apriltag_multiscale(gray_image)

    注意：
        此函数返回的格式与 cv2.aruco.detectMarkers 完全兼容
        可以直接替换原有的检测调用
    """
    detector = AdvancedAprilTagDetector(
        verbose=verbose,
