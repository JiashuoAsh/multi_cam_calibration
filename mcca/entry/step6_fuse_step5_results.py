#!/usr/bin/env python3
"""Step 6: 融合多组 Step5 位姿文件（B_T_C，Cam->Base）（支持多相机）。

目的
- 用户会在不同“板位置/采集批次”重复运行 Step5b，得到多份位姿文件 `camera_poses_B_T_C.json`。
...
输入
- 多个 Step5 输出的 `camera_poses_B_T_C.json`（路径可为文件 / 目录 / glob）。

输出
- 默认：results/camera_poses_B_T_C_fused.json
- 同时生成一个简要报告：results/camera_poses_B_T_C_fusion_report.json

使用示例
- 融合两次运行：
    python -m mcca.entry.step6_fuse_step5_results \
    -i run1/results/camera_poses_B_T_C.json run2/results/camera_poses_B_T_C.json

- 输入目录（会递归搜索 camera_poses_B_T_C.json）：
    python -m mcca.entry.step6_fuse_step5_results -i ./archive

- 使用 glob：
    python -m mcca.entry.step6_fuse_step5_results -i "archive/**/camera_poses_B_T_C.json"
...
POSE_IN_NAME = "camera_poses_B_T_C.json"
POSE_OUT_DEFAULT = "results/camera_poses_B_T_C_fused.json"
REPORT_OUT_DEFAULT = "results/camera_poses_B_T_C_fusion_report.json"
POSE_OVERWRITE_PATH = "results/camera_poses_B_T_C.json"
...
def _ensure_transform(T: np.ndarray, name: str) -> np.ndarray:
    ...
    ortho_err = float(np.linalg.norm(Rm.T @ Rm - np.eye(3), ord="fro"))
    if ortho_err > 5e-2:
    ...
    for s in inputs:
        if any(ch in s for ch in ["*", "?", "["]):
            for p in sorted(glob.glob(s, recursive=True)):
                if os.path.isdir(p):
                    for q in sorted(glob.glob(os.path.join(p, "**", POSE_IN_NAME), recursive=True)):
                        add(q)
                else:
                    add(p)
            continue
        ...
        if os.path.isdir(s):
            for p in sorted(glob.glob(os.path.join(s, "**", POSE_IN_NAME), recursive=True)):
                add(p)
        else:
            add(s)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m mcca.entry.step6_fuse_step5_results",
        description="Step6: 融合多组 Step5 位姿文件（camera_poses_B_T_C.json）",
    )
    parser.add_argument(
        "-i",
        "--inputs",
        nargs="+",
        required=True,
        help=f"输入文件/目录/glob（会递归搜 {POSE_IN_NAME}）",
    )
    parser.add_argument(
        "--out",
        default=POSE_OUT_DEFAULT,
        help=f"融合结果输出路径（默认：{POSE_OUT_DEFAULT}）",
    )
    parser.add_argument(
        "--report",
        default=REPORT_OUT_DEFAULT,
        help=f"融合报告输出路径（默认：{REPORT_OUT_DEFAULT}）",
    )
    ...
    parser.add_argument(
        "--overwrite_results",
        action="store_true",
        help=f"将融合结果同时写入 {POSE_OVERWRITE_PATH}（覆盖）",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if args.overwrite_results:
        _save_json(POSE_OVERWRITE_PATH, fused_result)
    ...
    if args.overwrite_results:
        print(f"  [OK] 已覆盖写入: {POSE_OVERWRITE_PATH}")
    print("=" * 70)
