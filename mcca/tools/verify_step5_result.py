...
功能:
    1. 加载标定结果 (results/camera_poses_B_T_C.json)
    2. 实时读取相机画面
    3. 检测 AprilTag 标定板
    4. 计算并显示标定板在底盘坐标系中的位置 (X, Y, Z)

使用方法:
    1. 运行脚本: python -m mcca.tools.verify_step5_result
...
from mcca.adapters.camera_wrapper import init_camera
...
from mcca.core.detection import detect_apriltag_corners, get_detection_settings
...
def load_calibration_results(*, camera: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not os.path.exists("results/camera_poses_B_T_C.json"):
        raise FileNotFoundError("未找到 results/camera_poses_B_T_C.json，请先运行 step5b")

    with open("results/camera_poses_B_T_C.json", "r", encoding="utf-8") as f:
        base_calib = json.load(f)

    if "B_T_C" not in base_calib or not isinstance(base_calib["B_T_C"], dict):
        raise ValueError("results/camera_poses_B_T_C.json 缺少 B_T_C 字段（请使用新版 step5b 重新生成）")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="python -m mcca.tools.verify_step5_result",
        description="验证 Step5：相机到底盘外参（支持无显示/headless 模式）",
    )
    parser.add_argument(
        "--camera",
        default="cam0",
    ...
    try:
        ...
        while True:
            ...
            success, rvec, tvec = estimate_pose_apriltag(corners, ids, obj_points, tag_ids, K, dist)
            ...
            if args.save_every and args.save_every > 0 and (frame_count % args.save_every == 0):
                ...
                out_img = cv2.resize(img, (0, 0), fx=s, fy=s) if s != 1.0 else img
        ...
        if detected_rows > 0:
            ...
            print(
                f"角度均值 (deg): roll={rpy_mean[0]:.2f}, pitch={rpy_mean[1]:.2f}, yaw={rpy_mean[2]:.2f}"
            )
            print(
                f"角度Std  (deg): roll={rpy_std[0]:.2f}, pitch={rpy_std[1]:.2f}, yaw={rpy_std[2]:.2f}"
            )
