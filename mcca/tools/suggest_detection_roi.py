...
动机：
- 小Tag 检测的第一杀手是“有效像素太少”。
- 直接整图 2x/3x 会很慢且更容易被背景干扰。
- 先锁定标定板所在区域（ROI），再在 ROI 上做更激进的多尺度/上采样，召回率会明显上升。

用法示例：
    python -m mcca.tools.suggest_detection_roi \
        --image images/raw/cam0/frame_000120.png --camera cam0 --write
...
from mcca.core.detection import create_detector_params, detect_apriltag_corners
...
def main() -> int:
    ap = argparse.ArgumentParser(
        prog="python -m mcca.tools.suggest_detection_roi",
        description="根据样例图自动建议 AprilTag 检测 ROI",
    )
    ...
    if corners is None or ids is None or len(ids) == 0:
        print(
            "无法检测到任何 tag：建议先用更近/更清晰的样例图生成 ROI；"
            "或先在 config 里把 profile 设为 small_tags 再试。"
        )
        return 2
